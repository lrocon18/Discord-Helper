...
def _human_click(ic, double: bool = False, right: bool = False) -> None:
    # Click in place — no pre-click drift.
    if ic is None:
        return
...
def _idle_tick() -> None:
    ...
    while not state.stop:
        ...
        while time.monotonic() < deadline and not state.stop:
            if not state.active or state.syncing:
                interrupted = True
                break
            time.sleep(0.1)
...
def _sig_a(c) -> bool:
    r, g, b = c
    # PT EU bar has gradient — mid-bar reads ~r=153, not 255. Floor at 80.
    return r > 80 and r > g * 2.5 and r > b * 2.5
...
def _aux_loop() -> None:
    jitter   = random.gauss((_AUX_JIT_LO + _AUX_JIT_HI) / 2, (_AUX_JIT_HI - _AUX_JIT_LO) / 4)
    jitter   = max(_AUX_JIT_LO, min(_AUX_JIT_HI, jitter))
    deadline = time.monotonic() + jitter   # first use: only jitter (assume skill ready)
    ...
    while not state.stop:
        ...
        if _ic and _can_tick():
            _ic.send_key('f2', hold_sec=random.uniform(0.08, 0.14))
            _sleep_human(0.5, 0.75)
            _human_click(_ic, double=False, right=True)
            _sleep_human(0.5, 0.75)
            _ic.send_key('f1', hold_sec=random.uniform(0.08, 0.14))
            jitter   = random.gauss((_AUX_JIT_LO + _AUX_JIT_HI) / 2, (_AUX_JIT_HI - _AUX_JIT_LO) / 4)
            jitter   = max(_AUX_JIT_LO, min(_AUX_JIT_HI, jitter))
            deadline = time.monotonic() + _AUX_CD + jitter
            _log(f"[MON] aux fired, next +{_AUX_CD + jitter:.1f}s")
...
def _sync_scheduler() -> None:
    ...
    while not state.stop:
        wait = _SYNC_INTERVAL + random.uniform(_JITTER_LO, _JITTER_HI)
        deadline = time.monotonic() + wait
...
def main() -> None:
    ...
    _log("[SVC] hotkeys register (low level)")
    import keyboard
    keyboard.add_hotkey('ctrl+k', _toggle_from_arduino)
    _log("[SVC] message loop (keyboard wait)")

    def _kill_from_keyboard():
        _log("[EVT] close requested (Ctrl+L)")
        state.stop = True
        _beep(400, 300)  # synchronous final beep
    keyboard.add_hotkey('ctrl+l', _kill_from_keyboard)
